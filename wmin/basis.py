    ...
    if overwrite_non_gluon_from_lhapdf:
        try:
            ...
            try:
                ...
                # Determine dimensions from NN output (this defines replica count and flavour channels).
                nreplicas, nflavours, _nx = pdf_array_nn.shape
                xgrid = np.asarray(LHAPDF_XGRID, dtype=float)
                if _nx != len(xgrid):
                    raise ValueError(
                        f"NN output nx={_nx} does not match LHAPDF_XGRID length={len(xgrid)}"
                    )

                # Flavour labels must align to the NN output ordering.
                model_flav_info = (
                    getattr(n3fit_pdf_model, "flav_info", None) or FLAV_INFO
                )
                model_flavs = [fi.get("fl") for fi in model_flav_info]

                if len(model_flavs) != nflavours:
                    log.warning(
                        "Flavour dimension mismatch: NN output has %s flavours but flav_info has %s. Extra channels will be frozen to NN replica-0 values.",
                        nflavours,
                        len(model_flavs),
                    )

                # Pad with None so we can handle extra NN output channels gracefully.
                if len(model_flavs) < nflavours:
                    model_flavs = model_flavs + [None] * (nflavours - len(model_flavs))

                # Fill baseline from LHAPDF for the flavours we can name.
                baseline = np.zeros((nflavours, len(xgrid)), dtype=pdf_array_nn.dtype)

                # Minimal translation between EVOL labels and colibri mapping keys.
                evol_to_colibri = {
                    "sng": "\\Sigma",
                    "v": "V",
                    "v3": "V3",
                ...
                for flav_index in range(nflavours):
                    flav_name = model_flavs[flav_index]
                    if flav_name is None:
                ...
                if "g" not in model_flavs:
                    raise ValueError(
                        f"Could not find gluon ('g') in flav_info. flav_info={model_flavs}"
                    )
                gluon_index = model_flavs.index("g")
