...
def n3fit_pdf_grid(
    ...
    while filter_integrability:
        v_grid = pdf_array[:, FLAVOUR_TO_ID_MAPPING["V"], :]        # extract V flavour from all replicas
        mask = np.abs(v_grid[:, :20].sum(axis=1)) <= integrability_threshold  # Sum first 20 x-points
        # Count how many replicas are being discarded
        n_discarded = (~mask).sum()  # Count False values
        n_kept = mask.sum()          # Count True values
        log.info(f"Filtering V integrability: discarded {n_discarded} replicas, kept {n_kept}")
        ...
        n_kept = mask.sum()
        log.info(f"Filtering V3 integrability: discarded {n_discarded} replicas, kept {n_kept}")        
        ...
        n_discarded = (~mask).sum()
        n_kept = mask.sum()
        log.info(f"Filtering V8 integrability: discarded {n_discarded} replicas, kept {n_kept}")        
        pdf_array = pdf_array[mask, :, :]
        ...
        n_discarded = (~mask).sum()
        n_kept = mask.sum()
        log.info(f"Filtering T3 integrability: discarded {n_discarded} replicas, kept {n_kept}")        
        pdf_array = pdf_array[mask, :, :]
        ...
        n_discarded = (~mask).sum()
        n_kept = mask.sum()
        log.info(f"Filtering T8 integrability: discarded {n_discarded} replicas, kept {n_kept}")        
        pdf_array = pdf_array[mask, :, :]
