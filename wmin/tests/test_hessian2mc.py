...
from wmin.hessian2mc import (
    write_mc_watt_thorne_replicas,
    write_hessian_to_mc_watt_thorne,
)
import pathlib


@mock.patch("wmin.hessian2mc.write_replica")
@mock.patch("wmin.hessian2mc.log.info")
def test_write_mc_watt_thorne_replicas(mock_log_info, mock_write_replica):
    ...
    replicas_df = pd.DataFrame(
        np.random.standard_normal(size=(num_nx_nq, n_all_eig)),
        columns=range(1, n_all_eig + 1),
    )
...
@mock.patch("wmin.hessian2mc.write_mc_watt_thorne_replicas")
@mock.patch("wmin.hessian2mc.load_all_replicas")
@mock.patch("wmin.hessian2mc.rep_matrix")
@mock.patch("wmin.hessian2mc.write_new_lhapdf_info_file_from_previous_pdf")
@mock.patch("wmin.hessian2mc.os.makedirs")
@mock.patch("wmin.hessian2mc.lhapdf.paths")
@mock.patch("wmin.hessian2mc.PDF")
