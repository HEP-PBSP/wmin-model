...
@pytest.mark.parametrize(
    "pdf_basis", ["intrinsic_charm", "perturbative_charm"]
)
def test_wmin_pdfbasis_normalization(pdf_basis):
    """
    Test the normalization of the PDF basis.
    """
    pdf_grid = np.random.rand(14, 50)

    pdf_grid_normalized = wmin_pdfbasis_normalization(pdf_grid=pdf_grid, pdf_basis=pdf_basis)
    ...
    if pdf_basis == "intrinsic_charm":
        assert_allclose(pdf_grid_normalized[FLAVOUR_TO_ID_MAPPING["V15"],:], valence)
        assert_allclose(pdf_grid_normalized[FLAVOUR_TO_ID_MAPPING["V24"],:], valence)
        assert_allclose(pdf_grid_normalized[FLAVOUR_TO_ID_MAPPING["V35"],:], valence)
